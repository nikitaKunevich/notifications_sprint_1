...
def send_notification(target: str, channel: str, subject: str, body: str) -> tuple[bool, str]:
...
def render_template(template_string: str, template_data: dict) -> str:
    logger.debug(f'render_template: got {template_string=} with params: {template_data=}')
...
def main():
    ...
    while True:
        ...
        for notification in chunk:
            ...
            logger.debug(f"Processing notification {notification['scheduled_datetime']}:{notification['id']}"
                         f" to {notification['email']}"
                         # f" via {notification['channel']}"
            )

            template = db.get_template_by_id(notification['template_id'])
            rendered_body = render_template(template['html'], notification['template_data'])
            rendered_subject = render_template(template['subject'], notification['template_data'])
            ok, status = send_notification(notification['email'], 'email', rendered_subject, rendered_body)
            if not ok:
