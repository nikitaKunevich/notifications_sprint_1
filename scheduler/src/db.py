...
def read_notifications_chunk(size=100) -> list[dict]:
    ...
    query = select(notifications).with_for_update(skip_locked=True) \
        .where(notifications.c.status == NotificationStatuses.to_send,
               notifications.c.scheduled_datetime < datetime.datetime.now()).limit(size)
    resultset = _connection.execute(query)
    notifications_objects = resultset.mappings().all()
    n_ids = [n['id'] for n in notifications_objects]
    query = update(notifications).where(notifications.c.id.in_(n_ids)).values(status=NotificationStatuses.in_process)
...
def mark_notification_as_sent(notification_id):
    query = update(notifications).where(notifications.c.id == notification_id).values(status=NotificationStatuses.done)
    _connection.execute(query)
...
def drop_or_resend_notification(notification_id):
    ...
    if notification['retry_count'] == config.MAX_RETRY_COUNT:
        logger.info(f"Stop trying to send notification {notification_id}, as retry_count exceeded max")
        update(notifications).where(notifications.c.id == notification_id).values(status=NotificationStatuses.failed)
        return

    update(notifications).where(notifications.c.id == notification_id)\
        .values(status=NotificationStatuses.to_send, retry_count=notification['retry_count'] + 1)
